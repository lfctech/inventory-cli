...
This directory contains a throwaway Snipe-IT instance used for local e2e tests.

## Quick start

```bash
make docker-up   # Start Snipe-IT + MySQL + seeder
make test-e2e    # Run the live CLI smoke tests
...
```bash
SNIPEIT_IMAGE=snipe/snipe-it:latest-alpine make test-e2e
```
